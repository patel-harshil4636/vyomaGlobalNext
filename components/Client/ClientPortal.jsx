...
import { 
  ...
  FileText, Link as LinkIcon, Lock, Eye, BarChart3, Image as ImageIcon
} from "lucide-react";
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 space-y-8">
                    {[
                      ...
                      { title: "Upcoming Work", data: currentMonth.upcomingWork || [], theme: { line: "bg-blue-500", text: "text-blue-500", dot: "bg-blue-400" }, icon: FileText }
                    ].map((section, idx) => (
                      <div key={idx} className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 relative overflow-hidden">
                        <div className={`absolute top-0 left-0 w-1 h-full ${section.theme.line}`}></div>
                        <h3 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-3"><section.icon className={`w-6 h-6 ${section.theme.text}`} />{section.title}</h3>
                          ...
                          <ul className="space-y-3">
                            {section.data.map((task, i) => (
                              <li key={i} className="bg-slate-50 rounded-2xl p-4 text-sm font-medium text-slate-700 flex items-start gap-3 border border-slate-100">
                                <div className={`w-1.5 h-1.5 rounded-full ${section.theme.dot} mt-1.5 flex-shrink-0`}></div> {task.task}
                              </li>
                            ))}
                          </ul>
                        )}
